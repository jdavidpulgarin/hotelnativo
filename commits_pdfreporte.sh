#!/bin/bash
# Script para generar commits incrementales de PdfReporteService.java
# Uso: bash commits_pdfreporte.sh

FILE="src/main/java/com/hotel/service/PdfReporteService.java"
TEMP="/tmp/.pdfreporte_full.java"

# Paso 0: Guardar el archivo completo en temp
cat > "$TEMP" << 'FULLEOF'
...
echo "[SCRIPT] Archivo completo guardado en temp"

# Ahora hacer commits incrementales usando line ranges del archivo completo
# Cada commit copia N lineas del archivo completo + cierra la clase

# Commit 1: imports + constantes + constructor (lineas 1-49 + cierre)
head -n 54 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: crear clase PdfReporteService con constantes y constructor"

# Commit 2: + generarFacturaPdf (lineas 1-80 + cierre)
head -n 84 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: implementar generarFacturaPdf en PdfReporteService"

# Commit 3: + generarReporteOcupacionPdf (lineas 1-134 + cierre)
head -n 144 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: implementar generarReporteOcupacionPdf en PdfReporteService"

# Commit 4: + generarReporteIngresosDiariosPdf (lineas 1-179 + cierre)
head -n 198 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: implementar generarReporteIngresosDiariosPdf en PdfReporteService"

# Commit 5: + generarReporteIngresosMensualesPdf (lineas 1-224 + cierre)
head -n 248 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: implementar generarReporteIngresosMensualesPdf en PdfReporteService"

# Commit 6: + generarReporteIngresosAnualesPdf (lineas 1-268 + cierre)
head -n 297 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: implementar generarReporteIngresosAnualesPdf en PdfReporteService"

# Commit 7: + agregarHeaderFact (lineas 1-303 + cierre)
head -n 348 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar header de factura PDF en PdfReporteService"

# Commit 8: + agregarLineaSep + agregarInfoFact (lineas 1-345 + cierre)
head -n 406 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar separador e info de facturacion en PdfReporteService"

# Commit 9: + agregarDetallesFact (lineas 1-380 + cierre)
head -n 454 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar detalles de servicios en factura PDF"

# Commit 10: + agregarTotalesFact (lineas 1-408 + cierre)
head -n 485 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar totales con IVA en factura PDF"

# Commit 11: + agregarFooterFact + agregarDisclaimer (lineas 1-434 + cierre)
head -n 520 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar footer y disclaimer en factura PDF"

# Commit 12: + agregarFilaKV + fmtCOP (lineas 1-449 + cierre)
head -n 539 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar helpers agregarFilaKV y fmtCOP en PdfReporteService"

# Commit 13: + crearRuta + agregarHeaderHotel (lineas 1-475 + cierre)
head -n 572 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar crearRuta y header de hotel para reportes PDF"

# Commit 14: + crearTabla + agregarEncabezado + agregarCelda (lineas 1-498 + cierre)
head -n 601 "$TEMP" > "$FILE"
echo "}" >> "$FILE"
git add . && git commit -m "feat: agregar helpers crearTabla, agregarEncabezado y agregarCelda"

# Commit 15: archivo completo (agregarKPI + agregarPiePagina + abrirPdf)
cp "$TEMP" "$FILE"
git add . && git commit -m "feat: agregar agregarKPI, agregarPiePagina y abrirPdf en PdfReporteService"

# Limpiar temp
rm -f "$TEMP"

# Push final
git push origin feature/lider-logica-negocio

echo ""
echo "=== LISTO! 15 commits de PdfReporteService creados y pusheados ==="
